...
`prism::PitchShifter` は `kShiftCentsMin = -150` で clamp する。UI の「スライダの範囲」は
**スライダの粒度**を変えるものであって、DSP が受け付ける範囲を広げるものではない。
全音(±200)やオクターブ(±1200)を選んで −150 を下回る値にすると、実際の音は
−150 セントで頭打ちになる。この差は隠さず、カード内に注記として出す。
