...
| パス | 内容 |
|---|---|
| `dsp/include/prism/PitchShifter.h` | `prism::PitchShifter`(**既定**)。ディレイライン型。ヘッダオンリー・依存ゼロの純 C++17。音声経路に FFT なし |
| `dsp/include/prism/PhaseVocoderShifter.h` | `prism::PhaseVocoderShifter`(**選択式の第 2 方式**)。位相ロック付き位相ボコーダ。遅延制約の緩い経路専用 |
...
| `pv-pitch@<fs>/<f>Hz/<±c>c` | **第 2 方式**。出力 FFT ピーク比が `2^(c/1200)` ±0.5% | あり |
| `pv-latency@<fs>/<±c>c` | **第 2 方式**。トーンバーストのエネルギー重心移動が `getLatencySamples()` と ±hop/4 以内、かつ ≤100 ms | あり |
| `pv-glitch@<fs>/<±c>c` | **第 2 方式**。既存と同じ不連続検出。0 件であること | あり |
| `pv-contract@<fs>/*` | **第 2 方式**の契約と境界条件(不正引数の拒否、FFT 長の丸め、NaN 無視/クランプ、`numFrames<=0`、dryWet=0 のビット一致素通し) | あり |
| `quality-chord@<fs>/*` | 品質指標。和音の期待スペクトル外エネルギー比。**報告のみ** | なし |
| `quality-chirp@<fs>/*` | 品質指標。チャープの期待位相からのずれ。**報告のみ** | なし |
| `cpu@<fs>/<±c>c/pv` | **第 2 方式**の処理時間 / 実時間。報告のみ | なし |
...
## 第 2 方式: 位相ボコーダ(`prism::PhaseVocoderShifter`)

### 目的

**既定は今までどおりディレイライン型**(`prism::PitchShifter`)。マイク経路では
イヤホンから生音が漏れ込むため、遅延 10 ms 以下が絶対条件であり FFT 系は使えない。

一方、**他アプリの再生音を捕獲する経路には生音の漏れ込みが無い**。この経路に限り
遅延 50〜100 ms を許容してよく、そこでは音質を優先できる。実機(Pixel 9a)で音楽を
−89 セント処理したときに常時聞こえる「音に張り付いたザラザラした粒ノイズ」は、
ディレイライン型の跳躍とクロスフェードに由来する。STFT 系では原理的に出ない。

CLAUDE.md「最重要制約」の例外節(2026-09-16 決定)に基づく**選択式**の方式であり、
既定にはならない。切替はユーザーの明示操作による。`prism::PitchShifter` は 1 行も変えていない。

### アルゴリズム

ピッチシフト = 時間伸縮(位相ボコーダ)+ 再サンプリング、という標準構成。
比は `r = 2^(cents/1200)`(半音 = 100 セントの決め打ちはしない)。

1. **STFT**。FFT 長 `N` は `prepare()` で 1024 / 2048 / 4096 から選択(既定 2048)。
   合成ホップ `Hs = N/4` 固定、解析ホップ `Ha = Hs / r`。伸縮率は `Hs/Ha = r`。
   解析・合成とも Hann 窓で、`Hs = N/4` のとき `Σ w² = 1.5`(厳密な COLA)なので
   `1/1.5` で正規化する。FFT はヘッダ内に閉じた自前 radix-2(テーブルは `prepare` で確保)。
2. **位相伝播**。bin ごとに期待位相進み `2π·m·Ha/N` からの偏差を主値化して瞬時周波数を
   求め、合成ホップぶん進める。
3. **位相ロック(Laroche–Dolson identity phase locking)**。振幅スペクトルのピーク bin を
   検出し、**ピークだけ**を瞬時周波数で伝播させる。ピークの影響圏(隣接ピークとの中点まで)
   の bin には、ピークと同じ位相回転量を与える:
   `outPhase[m] = phase[m] + (sumPhase[peak] − phase[peak])`。
   1 本の正弦波を構成する複数 bin の位相関係が壊れなくなり、位相ボコーダ特有の
   「水っぽさ / リバーブ感」が大きく減る。
4. **再サンプリング**。伸縮された信号を速度 `r` で読む(4 点 3 次 Hermite 補間)。
   音高が `r` 倍になり、長さが `1/r` 倍に戻る。差し引きで長さは元どおり。
5. **ナイキスト超えの除去**。上げ方向(`r > 1`)では `fs/(2r)` より上の成分が再サンプリングで
   折り返すため、合成前に `m > N/(2r)` の bin を 0 にする。ディレイライン型と違い
   アンチエイリアスが構造的に入る。

`process()` は任意の `numFrames`(≤ `maxBlock`)を受け、内部で固定長の入力 FIFO /
伸縮ストリーム FIFO を介して hop 単位に STFT を回す。**確保・ロック・I/O・例外なし**、
パラメータは `std::atomic<float>`(ロックフリーを `static_assert`)。API 形状は
`prism::PitchShifter` と同じなので、呼び出し側は差し替えるだけで切り替えられる。

### 遅延の式と実測

```
L(r) = P + (N/2)(1/r − 1)   [サンプル]      P = N + Hs = 1.25 N
```

`P` は STFT の充填ぶんの先読み(`prepare` で決まる固定量)。第 2 項は時間伸縮の基準点が
解析窓の中心にあることから来る群遅延で、下げ方向(`r<1`)では正、上げ方向では負になる。
`getLatencySamples()` はこの式そのものを返す。

実測は 440 Hz トーンバースト(無音 → 150 ms → 無音)の**出力と入力のエネルギー重心の差**。
インパルス応答の重心だと、位相ボコーダが位相を書き換えることによる分散で大きなシフト量の
とき数百サンプルずれるため、包絡の移動量で群遅延を測っている。許容差は `hop/4` = 128 サンプル。

| シフト | 設計値(サンプル) | @44.1k 実測 / ms | @48k 実測 / ms |
|---|---|---|---|
| −89(既定) | 2614.0 | 2614.0(差 +0.0)/ **59.27 ms** | 2614.2(差 +0.2)/ **54.46 ms** |
| −1200 | 3584.0 | 3574.5(差 −9.5)/ 81.27 ms | 3643.9(差 +59.9)/ 74.67 ms |
| +1200 | 2048.0 | 2048.0(差 −0.0)/ 46.44 ms | 2048.0(差 +0.0)/ 42.67 ms |

N=2048 / 48 kHz では全シフト量で 100 ms 以下(最大 74.67 ms @ −1200)。
なお `dryWet` の dry 側は wet と**同じ遅延に整合させて**読む(ディレイライン型と違う点)。
50 ms の素通しが重なると明確なエコーになるため。`pv-contract@*/drywet-bypass` が
`dryWet=0` でビット一致の `P` サンプル遅延素通しになることを検査しており、遅延量の独立検証にもなっている。

### ピッチ精度(全点 ±0.5% 以内)

値は測定誤差(cents)。許容差は全点 ±0.5%(= ±8.66 cents)。

| cents | −89 | −1200 | +100 | +1200 |
|---|---|---|---|---|
| 110 Hz @44.1k | +1.11 | +1.33 | +0.83 | −0.19 |
| 110 Hz @48k | −1.27 | +1.17 | +0.66 | −0.48 |
| 440 Hz @44.1k | +0.16 | −0.19 | −0.24 | +0.08 |
| 440 Hz @48k | −0.32 | −0.48 | −0.26 | +0.14 |
| 3520 Hz @44.1k | −0.02 | +0.07 | −0.00 | +0.00 |
| 3520 Hz @48k | +0.03 | −0.01 | +0.02 | +0.00 |

最悪 1.33 cents(0.077%)で、許容差の 1/6。グリッチは全ケース 0 件、確保も全ケース 0。

### 品質指標(報告のみ)— 期待スペクトル外エネルギー比

理想出力と実出力の差を、両方式を**同じ物差し**で並べた数値。小さいほど良い。

- **和音** 220+277+330 Hz を −89 セント。理想出力は 3 本の線スペクトルだけになる。
  期待ピーク周辺(±4 bin)以外に現れたエネルギーの比率。窓長は 2^18(約 6 秒)。
  ディレイライン型の跳躍・クロスフェードは跳躍間隔(走査幅 40 ms / −89 セントで約 760 ms)の
  遅い変調として現れるため、既存検査の 32768 点(約 0.7 秒)では搬送波の主ローブに埋もれて見えない。
- **チャープ** 200→2000 Hz / 2 秒を −89 セント。期待位相
  `φ(i) = 2π·r·(f0·t + slope·t²/2)/fs`(`t = i −` そのシフタの遅延)で複素復調してから
  2^16 点の複素 FFT。理想出力は DC の線になり、期待からのずれだけが DC 周辺に散らばる。
  分母は搬送波近傍 ±200 Hz に限る(復調で生じる和周波数成分を除くため)。

| 信号 | ディレイライン(走査 40 ms) | 位相ボコーダ N=2048 | 位相ボコーダ N=4096 |
|---|---|---|---|
| 和音 @44.1k | 7.78e−4 | 3.31e−1 | **1.9e−5** |
| 和音 @48k | 8.20e−4 | 2.34e−1 | **1.3e−5** |
| チャープ @44.1k | 9.91e−1 | **7e−6** | — |
| チャープ @48k | 9.88e−1 | **2e−6** | — |

読み方:

- **チャープでは位相ボコーダが 5 桁良い**。ディレイライン型の 0.99 は「ほぼ全エネルギーが
  期待周波数から外れた」という意味で、バグではなく方式の帰結 — 読み出しヘッドが 40 ms の
  幅を走査するため、非定常信号では読み出し時点の瞬時周波数がずれ、
  `slope × 20 ms = ±18 Hz` のこぎり状の周波数誤差が出る。グリッサンド・ビブラート・
  音楽全般が「ザラつく」原因の正体がこれ。
- **和音では `N` を上げる必要がある**。`N=2048` は 44.1 kHz で bin 幅 21.5 Hz、
  220/277/330 Hz は 2〜3 bin しか離れておらず、ピーク検出が 3 本を分離できない。
  そのとき identity phase locking は 3 本を 1 つの影響圏にまとめてしまい、非ピークの
  partial は「同じ比」ではなく「ピークと同じ**絶対 Hz**」だけずれる(277 Hz を基準に
  −13.9 Hz なら 220 → 206.1 Hz、正しくは 208.98 Hz)。これは identity phase locking の
  既知の限界であり実装のバグではない。`N=4096` にすると 3 本が分離し、
  ディレイライン型より **40 倍以上**良くなる。
  → **低音域の和音(音楽)には `N=4096` を使うこと。**

### CPU 比(Apple M2 / Apple clang 17 / −O2、実時間比)

| シフト | ディレイライン @44.1k / @48k | 位相ボコーダ N=2048 @44.1k / @48k |
|---|---|---|
| −89(既定) | 0.27 % / 0.49 % | 1.08 % / 1.14 % |
| −1200 | 2.05 % / 4.25 % | 0.62 % / 0.64 % |
| +1200 | 3.91 % / 8.42 % | — |

位相ボコーダの負荷は**シフト量にほぼ依存しない**(むしろ −1200 では解析ホップが
`Hs/r = 2Hs` に伸びてフレーム数が半減するので軽くなる)。ディレイライン型は跳躍頻度が
`|1−ratio|` に比例して相関探索が走るため、定義域の端で跳ね上がる。既定 −89 セントでは
位相ボコーダのほうが 2 倍強ほど重いが、いずれも実時間の 1.2 % 未満。
`N=4096` はフレームあたりの FFT が 2 倍・フレーム数が半分なので、おおむね同等〜微増。

### 向く経路 / 向かない経路

| 経路 | 方式 | 理由 |
|---|---|---|
| マイク(外界音)| **ディレイライン型のみ** | 生音が漏れ込む。遅延 10 ms 超で唸り・エコー・会話の破綻 |
| 他アプリ音の捕獲(音楽・動画)| **位相ボコーダ推奨**(`N=4096`)| 漏れ込み無し。50〜100 ms を許容して音質を取れる |
| 会話・環境音の捕獲 | どちらでも | 非定常成分が多いので位相ボコーダ有利だが、遅延が気になるなら `N=1024`(遅延 1.25×1024 = 26.7 ms @48k)|
...
第 2 方式(`prism::PhaseVocoderShifter`)固有:

- **マイク経路には使えない**。最小でも `N=1024` で遅延 26.7 ms @48k。生音の漏れ込みがある
  経路では唸り・エコーになる。既定は常にディレイライン型。
- **低音域の和音は `N=2048` では精度が落ちる**。bin 幅より partial 間隔が狭いと
  identity phase locking がそれらを 1 つの影響圏にまとめ、非ピーク partial が
  「同じ比」ではなく「絶対 Hz」でずれる(上表の和音 @N=2048 が 0.23〜0.33)。
  `N=4096` で解消する。音楽用途では `N=4096` を既定にすること。
- **遅延はシフト量に依存する**(`L(r) = P + (N/2)(1/r − 1)`)。−1200 セントで
  74.7 ms @48k、+1200 セントで 42.7 ms @48k。実行中にセント値を大きく動かすと
  遅延も動く — 平滑はブロック単位で入れてあるが、遅延そのものは連続には変わらない。
- **過渡音がにじむ**。位相ボコーダ一般の性質で、打楽器のアタックが N/4 程度ぼける。
  シフト量が小さいほど目立たない(既定 −89 セントでは実用上問題にならない範囲)。
- **`dryWet` の dry 側は遅延整合される**(ディレイライン型は素通し)。`dryWet<1` にしても
  エコーにはならないが、原音そのものも `L` サンプル遅れる。
